...
function createMarkup(arr) {
  ...
  <a class="gallery-link" href="${item.original}">
    <img
      class="gallery-image custom-cursor"
      src="${item.preview}"
...
function handleItemClick(event) {
  if (event.target.nodeName === 'IMG') {
    event.preventDefault(); // preventing from auto downloading image
  }
  if (event.target === event.currentTarget) {
    return; // user didn't hit image
  }

  const currentImage = event.target.closest('.gallery-image');
  const imageLink = currentImage.dataset.source;
  const imageObj = images.find(item => item.original === imageLink);
